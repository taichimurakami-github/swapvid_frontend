...
export function usePreGeneratedScrollTimeline(
  ...
  const getActiveVideoViewportFromCurrentTime = useCallback(
    (currentTime: number): TBoundingBox | null => {
      const currentSection =
        documentTimeline.find(
          (v) => v.time[0] <= currentTime && currentTime < v.time[1]
        ) ?? null;

      return currentSection?.videoViewport ?? null;
    },
    [documentTimeline]
  );
...
export function usePreGeneratedActivityTimeline(
